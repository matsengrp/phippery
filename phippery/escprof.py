...
import xarray as xr
import ot
from phippery.utils import peptide_id_coordinate_subset
from phippery.utils import sample_id_coordinate_subset
...
def get_loc_esc_distr(
    ...
    my_ds = ds.loc[
                dict(
                    ...
                    sample_id=sample_id_coordinate_subset(ds,where=sample_factor,is_equal_to=sfact_val)
                )
            ]
...
def get_weights(
    ...
    for loc in range(loc_start, loc_end+1):
        ds1 = ds.loc[
                dict(
                    peptide_id=peptide_id_coordinate_subset(ds,where='Loc',is_equal_to=loc),
                    sample_id=sample_id_coordinate_subset(ds,where=sample_factor,is_equal_to=sfact_val1)
                    )
                ]
        ...
        ds2 = ds.loc[
                dict(
                    peptide_id=peptide_id_coordinate_subset(ds,where='Loc',is_equal_to=loc),
                    sample_id=sample_id_coordinate_subset(ds,where=sample_factor,is_equal_to=sfact_val2)
                    )
                ]
